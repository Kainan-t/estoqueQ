...
import type { SaldoPF } from '@/types'
...
export function ExpedicaoForm({ produtoId, saldo, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (v + a + r === 0) {
      setError('Informe ao menos uma caixa para expedir.')
      return
    }
    if (v > saldo.cx_verdes) {
      setError(`Verdes insuficientes. Disponível: ${saldo.cx_verdes}`)
      return
    }
    if (a > saldo.cx_amarelas) {
      setError(`Amarelas insuficientes. Disponível: ${saldo.cx_amarelas}`)
      return
    }
    if (r > saldo.cx_vermelhas) {
      setError(`Vermelhas insuficientes. Disponível: ${saldo.cx_vermelhas}`)
      return
    }
    setLoading(true)
    try {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Saldo disponível: {saldo.cx_verdes} 🟢 · {saldo.cx_amarelas} 🟡 · {saldo.cx_vermelhas} 🔴
      </p>
      ...
      <div className="flex gap-2">
        <Button type="submit" disabled={loading} className="flex-1">
          {loading ? 'Salvando...' : 'Registrar Expedição'}
        </Button>
